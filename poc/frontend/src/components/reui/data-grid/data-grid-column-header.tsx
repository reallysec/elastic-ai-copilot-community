...
function DataGridColumnHeaderInner<TData extends object, TValue>({
  ...
  const headerLabelClassName = cn(
    "text-secondary-foreground/80 inline-flex h-full items-center gap-1.5 font-normal [&_svg]:opacity-60 text-13 leading-[calc(1.125/0.8125)] [&_svg]:size-3.5",
    className
  )
  ...
  const sortIcon =
    canSort &&
    (isSorted === "desc" ? (
      <HugeiconsIcon icon={ArrowDown02Icon} strokeWidth={2} className="size-3.25" aria-hidden="true" />
    ) : isSorted === "asc" ? (
      <HugeiconsIcon icon={ArrowUp02Icon} strokeWidth={2} className="size-3.25" aria-hidden="true" />
    ) : (
      <HugeiconsIcon icon={UnfoldMoreIcon} strokeWidth={2} className="mt-px size-3.25" aria-hidden="true" />
    ))
